...
export async function buildRelease({ root, allowDirty = false, runTests = true } = {}) {
  ...
  if (status.trim() && !allowDirty) throw new Error("Refusing to package a dirty tree. Commit the release first, or pass --allow-dirty.");
  const { stdout: sha } = await execFileAsync("git", ["rev-parse", "HEAD"], { cwd: root });
  const commit = sha.trim();
  if (runTests) { await inherit("npm", ["run", "check"], root); await inherit("npm", ["test"], root); }
  ...
  const { stdout } = await execFileAsync("npm", ["pack", "--json", "--pack-destination", outputDir], {
    cwd: root, env: { ...process.env, NPM_CONFIG_CACHE: path.join(root, ".cache", "npm-release") }, maxBuffer: 10 * 1024 * 1024
  });
  const packed = JSON.parse(stdout)[0];
  const artifact = path.join(outputDir, packed.filename);
  const bytes = await fs.readFile(artifact);
  const packageJson = JSON.parse(await fs.readFile(path.join(root, "package.json"), "utf8"));
  const manifest = { schemaVersion: 1, package: packageJson.name, version: packageJson.version, commit, dirty: !!status.trim(), artifact: packed.filename,
    sha256: crypto.createHash("sha256").update(bytes).digest("hex"), size: bytes.length,
    files: packed.files.map(({ path: file, size }) => ({ path: file, size })), builtAt: new Date().toISOString() };
