...
[[ "${QWEN4EXP_PLE_MMAP:-1}" == "0" ]] || { echo "Resident graph launcher requires GPU-resident PLE" >&2; exit 1; }

# Compose baseline and fusion guards; safe on pristine images and restarts.
python3 /opt/lloom/qwen-v8/apply-stack.py
python3 /opt/lloom/qwen-prefix/apply-prefix-fixes.py

# Per-rank KV budget in bytes; default 28 GiB (30064771072).
KV_CACHE_MEMORY_BYTES="${KV_CACHE_MEMORY_BYTES:-30064771072}"
[[ "$KV_CACHE_MEMORY_BYTES" =~ ^[1-9][0-9]*$ ]] || { echo "KV_CACHE_MEMORY_BYTES must be a positive integer" >&2; exit 1; }
...
if [[ "${ENABLE_PREFIX_CACHING:-0}" == "1" ]]; then
  cache_args=(--enable-prefix-caching --mamba-cache-mode align --prefix-cache-retention-interval "${PREFIX_CACHE_RETENTION_INTERVAL:-1600}")
fi
ep_args=()
