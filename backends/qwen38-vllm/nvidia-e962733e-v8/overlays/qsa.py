...
class Qwen4ExpQSAAttention(Qwen3NextAttention, AttentionLayerBase):
    ...
    def forward(
        ...
        # attn_output_gate is mandatory for this architecture; the fused
        # QSA kernels apply sigmoid(gate) in FP32 with BF16 attention-output
        # rounding preserved, so the outer Python sigmoid is removed.
