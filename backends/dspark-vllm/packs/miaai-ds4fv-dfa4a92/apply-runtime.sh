...
source "${pack_root}/launch-options.sh"
...
# Upstream 7440c53 opt-ins. Existing scheduler/tool guards remain in place.
if [[ "${DSPARK_ENABLE_DSML_RECOVERY:-0}" == "1" ]]; then
  python3 "${patches}/hotfix-vllm-dsml-recovery.py" --check
  python3 "${patches}/hotfix-vllm-dsml-recovery.py"
  python3 "${patches}/hotfix-vllm-dsml-recovery.py" --status
fi
if [[ "${DSPARK_ENABLE_MXFP4_INDEXER_CACHE:-0}" == "1" ]]; then
  python3 "${patches}/hotfix-vllm-mxfp4-indexer-cache.py" --check
  python3 "${patches}/hotfix-vllm-mxfp4-indexer-cache.py"
  python3 "${patches}/hotfix-vllm-mxfp4-indexer-cache.py" --status
fi
if [[ "${DSPARK_ENABLE_ISSUE144_EFFORT_ALIGN:-0}" == "1" ]]; then
  python3 "${patches}/hotfix-dsv4-issue144-effort-align.py" --check --target "${vllm_root}/tokenizers/deepseek_v4_encoding.py"
  python3 "${patches}/hotfix-dsv4-issue144-effort-align.py"
  python3 "${patches}/hotfix-dsv4-issue144-effort-align.py" --status
fi
...
# Upstream dfa4a92: metadata-local C128A reuse remains opt-in (upstream default 0).
if [[ "${DSPARK_ENABLE_C128A_PREFILL_CACHE:-0}" == "1" ]]; then
  python3 "${patches}/hotfix-vllm-c128a-prefill-cache.py" --check
  python3 "${patches}/hotfix-vllm-c128a-prefill-cache.py"
  python3 "${patches}/hotfix-vllm-c128a-prefill-cache.py" --status
fi
