...
try {
  ...
  const localAlias = recipe.models[0].aliases.find((alias) => alias.id === 'deepseek-v4-flash-0731');
  assert.deepEqual(localAlias.fallbacks, undefined);
  assert.match(localAlias.description, /strict local/i);
  ...
  const archivedV14Recipe = JSON.parse(
    readFileSync(
      path.join(repoRoot, 'recipes', 'archive', 'linux-nvidia-dgx-spark-2x-deepseek-v4-flash-mia-vllm', 'v14.json'),
      'utf8'
    )
  );
  ...
  assert.equal(indexEntry.currentVersion, 19);
  assert.deepEqual(
    ...
    [
      ...
      { version: 18, status: 'archived' },
      { version: 19, status: 'current' }
    ]
  );
