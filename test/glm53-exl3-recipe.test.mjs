...
const plan = planRecipe(recipe, { models: [], runtimes: {} }, {
  modelRoot: '/models',
  platform: 'linux',
  arch: 'arm64',
...
assert.deepEqual(
  plan.steps.filter((step) => step.action === 'download-model').map((step) => [step.model, step.revision, step.destination]),
  [
    ['Mia-AiLab/GLM-5.3-Flash-EXL3-TR3-4bpw', '25a44fdbf16862a46b7cc9921142c6c81350af2f', '/models/Mia-AiLab--GLM-5.3-Flash-EXL3-TR3-4bpw'],
    ['incoai/GLM-5.3-Flash-DFlash2', '7d74cdd881ed7e32c31175984a67823127b66cfe', '/models/incoai--GLM-5.3-Flash-DFlash2']
  ]
);

const members = recipe.models[0].settings.placement.members;
assert.deepEqual(members.map((member) => member.role), ['worker', 'head']);
...
for (const [name, expected] of Object.entries(sourceHashes)) {
  ...
  assert.equal(crypto.createHash('sha256').update(bytes).digest('hex'), expected, `${name} drifted from Mia f3043c95`);
}
