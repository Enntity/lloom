...
if (listened) {
  ...
  try {
    ...
    assert(dashboardHtml.includes('id="topology-view-mode"'));
    assert(dashboardHtml.includes('VIEW: COLUMNS'));
    assert(dashboardHtml.includes('id="topology-camera-state"'));
    assert(dashboardHtml.includes('topologyViewMode: "columns"'));
    assert(dashboardHtml.includes('setTopologyViewMode(state.topologyViewMode === "action" ? "columns" : "action")'));
    assert(dashboardHtml.includes('function updateActionModelLayout('));
    assert(dashboardHtml.includes('function modelActivityWeight('));
    assert(dashboardHtml.includes('function actionCameraZoom('));
    assert(dashboardHtml.includes('function actionLoomAnchor('));
    assert(dashboardHtml.includes('function actionModelSlot('));
...
if (mockListened) {
  ...
  if (streamListened) {
    ...
    try {
      ...
      assert.equal(typeof metricsJson.rolling.short.errors, 'number');
      assert.equal(metricsJson.rolling.short.windowMs, 10000);
      assert.equal(typeof metricsJson.rolling.minute.errors, 'number');
