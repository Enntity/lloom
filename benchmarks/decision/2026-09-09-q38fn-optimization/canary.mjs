...
if (phase === 'smoke') {
  ...
  const monitor = (async () => {
    while (monitoring) {
      ...
      const runtime = status.runtimeManager.runtimes[process.env.QWEN_RUNTIME || 'qwen38-flash-next-cluster'];
...
assert.equal(attributed.length, results.length);
if (process.env.EXPECTED_MODEL || model === 'q38fn-local')
  assert(
    attributed.every(
      (x) => x.status === 200 && x.resolvedModel === (process.env.EXPECTED_MODEL || 'qwen3.8-flash-next')
    ),
