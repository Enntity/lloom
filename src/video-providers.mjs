...
export async function generateProviderVideo({ backend, body, signal, fetchFn = fetch, sleepFn = ms => new Promise(r => setTimeout(r, ms)), pollMs = 8000, timeoutMs = 600000 }) {
  ...
  const request = async (url, method = 'GET', payload) => {
    ...
    const r = await fetchFn(url, { method, signal: boundedSignal, redirect: 'error', headers: { Authorization: `${provider === 'replicate' ? 'Token' : 'Bearer'} ${key}`, 'User-Agent': 'Mozilla/5.0 (compatible; LLooM/0.2)', 'Content-Type': 'application/json' }, ...(payload ? { body: JSON.stringify(payload) } : {}) });
  ...
  if (provider === 'replicate') {
    ...
    job = await request(`${base}/models/${body.model}/predictions`, 'POST', { input: { image: body.image, audio: body.audio, prompt: body.prompt, fast_mode: body.fast_mode === true } });
  } else {
    const { model, prompt, duration, resolution, aspect_ratio, frame_images } = body;
    if (!Array.isArray(frame_images) || !frame_images.length || frame_images.some(f => !String(f.image_url?.url || '').startsWith('data:image/'))) throw new Error('Frame image data URIs required');
    job = await request(`${base}/videos`, 'POST', { model, prompt, duration, resolution, aspect_ratio, frame_images, generate_audio: false });
  }
  if (!job.id) throw new Error('Video provider returned no job ID');
  const jobUrl = provider === 'replicate' ? `${base}/predictions/${encodeURIComponent(job.id)}` : `${base}/videos/${encodeURIComponent(job.id)}`;
  ...
  const output = provider === 'replicate' ? (Array.isArray(job.output) ? job.output[0] : job.output) : (job.unsigned_urls?.[0] || `${jobUrl}/content?index=0`);
  ...
  if (url.protocol !== 'https:' || !(url.origin === origin || (provider === 'replicate' ? replicateOutput : openrouterOutput))) throw new Error('Untrusted video output origin');
  const r = await fetchFn(url.href, { signal: boundedSignal, redirect: 'error', headers: url.origin === origin ? { Authorization: `Bearer ${key}` } : {} });
