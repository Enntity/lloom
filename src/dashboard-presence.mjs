...
export const presenceViews = `
    ...
    <section id="view-machines" class="presence-view" hidden>
      <div class="presence-heading"><div><h2>Better together.</h2><p>See the hardware behind your models.</p></div></div>
      <div id="presence-machines" class="presence-grid"></div>
      <h3 class="presence-section-title">Nearby machines</h3><div class="presence-message">Automatic discovery is coming. Your configured peers are shown above.</div>
    ...
    <div id="presence-add" class="presence-drawer" hidden><section class="presence-dialog" role="dialog" aria-modal="true" aria-labelledby="presence-add-title"><h2 id="presence-add-title">Add a model.</h2><p class="muted">Choose a recommended recipe or bring a model reference. LLooM checks the installation before applying it.</p><div id="presence-recommendation"></div><form id="presence-add-form"><label>Model reference<input id="presence-model-ref" required placeholder="Paste a model link or an Ollama tag" aria-describedby="presence-model-ref-hint"></label><p id="presence-model-ref-hint" class="muted">Hugging Face links must select a specific commit. Ollama tags, local paths, and compatible endpoints are also supported.</p><details><summary>Installation details</summary><label>Backend<input id="presence-model-backend" placeholder="Choose automatically"></label><label>Display name<input id="presence-model-name" placeholder="Optional"></label></details><div id="presence-add-review" hidden><p id="presence-plan-summary"></p><details><summary>Review full plan</summary><pre id="presence-plan-json"></pre></details></div><p id="presence-add-error" role="alert"></p><div class="actions"><button id="presence-add-close" type="button">Cancel</button><button type="submit" class="primary" id="presence-add-plan">Review installation</button><button type="button" class="primary" id="presence-add-apply" hidden>Install model</button></div></form></section></div>
`;
