...
- `settings`: runtime knobs consumed during config materialization, including `contextWindow`, `maxOutputTokens`, `maxActiveRequests`, `profile`, `draftDepth`, `reasoning`, `preserveThinking`, `batchingPreset`, `memoryGb`, `startupTimeoutMs`, `priority`, `keepWarm`, and session-cache fields. `keepWarm` is the single hard residency pin and is valid only for a managed internal runtime.
...
- `settings.placement`: optional cluster placement. `{"mode":"replicated"}` materializes one target on every configured node (or the `nodes` subset); distributed logical runtimes use ordered `members` with per-node resource estimates. See [`clusters.md`](clusters.md).
