...
In cluster mode, `/gateway/node` reports only the local node's host telemetry and physical runtimes, while `/gateway/cluster` fans out through authenticated LLooM admin endpoints and returns a node-keyed topology. `/gateway/status` includes that cluster graph plus logical replicated/distributed runtime state. Raw model backends stay on the private fabric; clients continue to use the leader gateway.
