...
- The Spark deployment config now pins every generated OMP role to the local `glm53-flash` alias and includes advertised aliases in client catalogs, so managed recipe updates cannot silently regenerate DeepSeek role bindings or omit the selected GLM alias from OMP's model catalog.
